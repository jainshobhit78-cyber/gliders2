...
<div class="about-section">
    <div class="title-header d-flex align-items-center justify-content-between">
        <h5 class="mb-0 page-title">Edit {{ $title }} Leader: {{ $leader->name }}</h5>
    ...
    <div class="container-fluid mt-3">
        <div class="card">
            <div class="card-body">
                <form method="POST" action="{{ $type === 'opf' ? route('admin.opf_legacy.update', $leader->id) : route('admin.legacy.update', $leader->id) }}" enctype="multipart/form-data">
                    @csrf
                        ...
                        <div class="tab-pane fade show active" id="basic-info" role="tabpanel">
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" name="name" class="form-control" value="{{ old('name', $leader->name) }}" placeholder="e.g. Shri A. K. Srivastava" required>
                                </div>
                                <div class="col-md-6 mb-3">
                                    <label class="form-label">Role <span class="text-danger">*</span></label>
                                    <input type="text" name="role" class="form-control" value="{{ old('role', $leader->role) }}" required>
                                </div>
                            </div>
                            ...
                            <div class="row">
                                <div class="col-md-4 mb-3">
                                    ...
                                    <input type="text" name="tenure_start" class="form-control" value="{{ old('tenure_start', $leader->tenure_start) }}" placeholder="e.g. 1992">
                                </div>
                                <div class="col-md-4 mb-3">
                                    <label class="form-label">Tenure End Year</label>
                                    <input type="text" name="tenure_end" class="form-control" value="{{ old('tenure_end', $leader->tenure_end) }}" placeholder="e.g. 1997 (leave empty for Present)">
                                </div>
                                <div class="col-md-4 mb-3">
                                    <label class="form-label">Tenure Label/Text</label>
                                    <input type="text" name="tenure_text" class="form-control" value="{{ old('tenure_text', $leader->tenure_text) }}" placeholder="e.g. Tenure: 1992 – 1997">
                                </div>
                            </div>
                            ...
                            <div class="row">
                                <div class="col-md-4 mb-3">
                                    ...
                                    <input type="text" name="initials" class="form-control" value="{{ old('initials', $leader->initials) }}" placeholder="e.g. AS" maxlength="5">
                                </div>
                                <div class="col-md-4 mb-3">
                                    <label class="form-label">Theme Color</label>
                                    <input type="color" name="color" class="form-control form-control-color w-100" value="{{ old('color', $leader->color ?: '#0b2a5b') }}">
                                </div>
                                <div class="col-md-4 mb-3">
                                    ...
                                    @if($leader->image)
                                        <div class="d-flex align-items-center gap-2">
                                            <img src="{{ asset('uploads/category/' . $leader->image) }}" class="rounded" style="height: 50px; width: 50px; object-fit: cover;">
                                            <span class="text-muted" style="font-size: 12px;">Current Image</span>
                                        </div>
                                    @endif
                                    <small class="text-muted d-block mt-1">Will show inside the avatar circle. Falls back to Initials if empty.</small>
                                </div>
                            </div>

                            <div class="mb-3">
                                <label class="form-label">Short Description</label>
                                <textarea name="description" class="form-control" rows="4" placeholder="Short biography or summary of accomplishments...">{{ old('description', $leader->description) }}</textarea>
                            </div>

                            <div class="mb-3">
                                <label class="form-label">Quote</label>
                                <textarea name="quote" class="form-control" rows="2" placeholder="e.g. Scaling capacity responsibly is the surest way to meet the nation's growing demand.">{{ old('quote', $leader->quote) }}</textarea>
                            </div>
                        </div>
...
<script>
    ...
    window.addEventListener('DOMContentLoaded', () => {
        let existingFocus = @json($leader->focus_areas);
        let existingStats = @json($leader->stats);
        let existingTimeline = @json($leader->timeline);

        if (typeof existingFocus === 'string') {
            try { existingFocus = JSON.parse(existingFocus); } catch(e) { existingFocus = []; }
        }
        if (typeof existingStats === 'string') {
            try { existingStats = JSON.parse(existingStats); } catch(e) { existingStats = []; }
        }
        if (typeof existingTimeline === 'string') {
            try { existingTimeline = JSON.parse(existingTimeline); } catch(e) { existingTimeline = []; }
        }

        existingFocus = existingFocus || [];
        existingStats = existingStats || [];
        existingTimeline = existingTimeline || [];

        if (existingFocus.length > 0) {
            existingFocus.forEach(f => addFocusRow(f.icon || f[0], f.label || f[1]));
        } else {
            addFocusRow('wrench', 'Capacity Expansion');
        }

        if (existingStats.length > 0) {
            existingStats.forEach(s => addStatRow(s.icon || s[0], s.number || s[1], s.label || s[2]));
        } else {
            addStatRow('rupee', '₹120 Cr+', 'Orders Secured');
        }

        if (existingTimeline.length > 0) {
            existingTimeline.forEach(t => addTimelineRow(t.year || t[0], t.title || t[1], t.icon || t[2]));
        } else {
